...
if __name__ == '__main__':
    ...
    if args.method == 'macenko':
        ...
        # print('Unsupported normalization method:', args.method)
        exit(-1)

    if not os.path.exists(target_img):
        print(f'Target image {target_img} not found')
        # print('Target image not found:', target_img)
        exit(-1)
        ...
        for dir in tqdm(dirs):
            ...
            for file in os.listdir(abs_path):
                ...
                if file.endswith('.jpg') or file.endswith('.png') or file.endswith('.jpeg'):
                    ...
                    if os.path.exists(os.path.join(args.output_dir, os.path.relpath(img_path, args.target_dir))):
                        print(f'{idx} / {length} already exists!')
                        idx += 1
                        continue
                    else:
